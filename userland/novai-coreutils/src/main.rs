...
enum Cmd {
    ...
    Tail  { file: PathBuf, #[arg(short='n', default_value_t = 10)] n: usize },
    /// Print first N lines of a file.
    Head  { file: PathBuf, #[arg(short='n', default_value_t = 10)] n: usize },
    /// Copy files.
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Rm{files}     => { for f in &files { let _ = fs::remove_file(f); } Ok(()) }
        Cmd::Mkdir{dirs}   => { for d in &dirs { fs::create_dir_all(d)?; } Ok(()) }
        Cmd::Pwd           => { println!("{}", std::env::current_dir()?.display()); Ok(()) }
        Cmd::Tail{file,n}  => tail(&file, n),
        Cmd::Head{file,n}  => head(&file, n),
        Cmd::Cp{from,to}   => { fs::copy(&from, &to)?; Ok(()) }
        Cmd::Mv{from,to}   => { fs::rename(&from, &to)?; Ok(()) }
        Cmd::Uptime        => uptime(),
        Cmd::Free          => free(),
    }
}
...
fn free() -> Result<()> {
    ...
    for line in raw.lines() {
        if let Some(v) = line.strip_prefix("MemTotal:") {
            total = v.split_whitespace().next().unwrap_or("0").parse().unwrap_or(0);
        }
        if let Some(v) = line.strip_prefix("MemAvailable:") {
            avail = v.split_whitespace().next().unwrap_or("0").parse().unwrap_or(0);
        }
    }
    println!("{:<12} {:>12} {:>12} {:>12}", "", "total", "used", "available");
    println!("{:<12} {:>12} {:>12} {:>12}",
             "Mem:",
             kb_to_human(total),
...
fn kb_to_human(kb: u64) -> String {
    if kb >= 1024 * 1024 { format!("{:.2}G", kb as f64 / (1024.0 * 1024.0)) }
