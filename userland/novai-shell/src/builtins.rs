...
pub fn is_builtin(name: &str) -> bool {
    matches!(name, "cd" | "pwd" | "exit" | "export" | "echo" | "set" | "alias" | "history" | "help" | "source" | "which" | "true" | "false")
}

pub fn run(argv: &[String]) -> anyhow::Result<i32> {
    match argv[0].as_str() {
        "cd"     => cd(argv),
        "pwd"    => { println!("{}", env::current_dir()?.display()); Ok(0) }
        "exit"   => { let code = argv.get(1).and_then(|s| s.parse::<i32>().ok()).unwrap_or(0); std::process::exit(code); }
        "export" => export(argv),
        "echo"   => { println!("{}", argv[1..].join(" ")); Ok(0) }
        "set"    => set(argv),
        "alias"  => { for a in &argv[1..] { println!("alias {}", a); } Ok(0) }
        "history"=> { println!("(history is in $XDG_CACHE_HOME/novai-shell-history.txt)"); Ok(0) }
        "source" => source(argv),
        "which"  => { for n in &argv[1..] { match which::which(n) { Ok(p) => println!("{}", p.display()), Err(_) => eprintln!("not found: {}", n) } } Ok(0) }
        "true"   => Ok(0),
        "false"  => Ok(1),
        "help"   => { print_help(); Ok(0) }
        _ => Ok(127),
    }
}
...
fn source(argv: &[String]) -> anyhow::Result<i32> {
    let path = argv.get(1).ok_or_else(|| anyhow::anyhow!("source: missing file"))?;
